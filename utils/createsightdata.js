...
let list = xlsx2json.parse(path.join(__dirname, '../data', fileName));
let data = [...(list[0].data[0])];
...
async function sailingInfo(conn, results) {
    ...
    results.forEach(result => {
        let createSailingInfo = [result.sailing_id, result.sighting_id, result.mix, result.dolphin_type, result.year, result.month, result.day, result.period, result.departure, result.arrival, result.boat_size, result.sighting, result.gps_no, result.guide, result.recorder, result.observations, result.weather, result.wind_direction, result.wave_condition, result.current]  
        sailingInfoArray.push(createSailingInfo)
    })
    let sqlSailingInfo = 'INSERT INTO sailing_info (sailing_id, sighting_id, mix, dolphin_type, year, month, day, period, departure, arrival, boat_size, sighting, gps_no, guide, recorder, observations, weather, wind_direction, wave_condition, current) VALUES ?'
...
async function obvDetail (conn, results) {
    ...
    results.forEach((result, obvId) => {
        let createObvDetail = [obvId + 1, result.sighting_method, result.dolphin_type, result.type_confirmation, result.dolphin_group_no, result.dolphin_type_no, result.dorsal_fin, result.exhalation, result.splash, result.exhibition, result.mother_child, result.mother_child_no, result.group_size_lowest, result.group_size_probable, result.group_size_highest, result.mix, result.mix_type]
...
async function obvApproach (conn, results) {
    let obvApproachArray = []
    results.forEach((result, obvId) => {
    let createObvApproach = [obvId + 1, result.approach_time, result.approach_gps_no, result.leaving_time, result.leaving_gps_no, result.leaving_method]
...
async function obvGps (conn, results) {
    let obvGPS = []
    results.forEach((result, obvId) => {
    let createObvGPS = [obvId + 1, result.latitude, result.latitude_min, result.latitude_sec, result.longitude, result.longitude_min, result.longitude_sec, result.boat_number]
